...
query_log = []  # per-query usage history

# GPT-4o pricing per 1M tokens
COST_PER_1M_INPUT  = 2.50
COST_PER_1M_OUTPUT = 10.00
COST_PER_1M_EMBED  = 0.10  # text-embedding-ada-002
...
def respond(message, history, subject):
    ...
    # Normalize history — handle both tuple and dict formats
    clean_history = []
    for h in history:
...
with gr.Blocks(title="📚 CBSE Class XII Learning Buddy") as demo:
    ...
    # Create token display first (rendered=False), mount it later at bottom
